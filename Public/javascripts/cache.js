class Cache {
    ...
    LRU_Policy(val) {
        ...
        this.fetchMap[this.storage[setNumber * this.associativity] - 1] = undefined;//putting the LRU block to undefined
        ...
        this.fetchMap[this.#blockNum(val)] = 1;//telling the Cache that this block exits in cache...
        this.storage[(setNumber + 1) * this.associativity - 1] = this.#blockNum(val) + 1;//assigning the MRU part of cache to curr block index+1
    ...
    RR_Policy(val) {//random replacement policy...
        ...
        this.fetchMap[this.storage[(setNumber) * this.associativity + ran] - 1] = undefined;//randomly evicting the block
        this.storage[(setNumber) * this.associativity + ran] = this.#blockNum(val) + 1//assigning that specific random location to new incoming block
    ...
    // checkStorage(index) {
    //     if (this.test_flag == false || !this.fetchMap[index]) {
    //         return false;
    //     }
    //     else {
    //         return true;
